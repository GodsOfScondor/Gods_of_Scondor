...
public class Loader {
	...
	public static int[] loadToVAO(float[] positions, int dimensions) {
		...
		unbindVAO();
		return new int[] {vaoID, positions.length/2};
	}
	...
	public static int[] loadTexture(String fileName) {
		...
		info[0] = textureID;
		info[1] = texture.getImageWidth();
		info[2] = texture.getImageHeight();
		return info;
	}
	...
	private static int storeDataInAttributeList(int attributeNumber, int x, float[] data) {
		int vboID = GL15.glGenBuffers();
		vbos.add(vboID);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboID);
		FloatBuffer buffer = storeDataInFloatBuffer(data);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
		GL20.glVertexAttribPointer(attributeNumber, x, GL11.GL_FLOAT, false, 0, 0);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
		return vboID;
	}
